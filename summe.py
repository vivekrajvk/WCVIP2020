...
def evaluateSummary(summary_selection,videoName,HOMEDATA):
     ...
     #print(len(summary_selection), nFrames)
     ...
     for userIdx in range(0,nbOfUsers):
         ...
         user_union[userIdx]=sum((list(map(lambda x: (1 if x>0 else 0), gt_indicator+summary_indicator))));         
     ...
     f_measure=sum(f_measure)/nbOfUsers;
...
def plotAllResults(summary_selections,methods,videoName,HOMEDATA):
    ...
    for methodIdx in range(0,len(methods)):
        ...
        for selIdx in summaryIndices:
            if selIdx>0:
                curSummary=list(map(lambda x: (1 if x>=selIdx else 0),summary_selections[methodIdx]))    
